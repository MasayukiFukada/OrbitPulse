...
export class SqliteBacklogRepository implements BacklogRepository {
  async findAll(): Promise<BacklogItem[]> {
    const results = await db.select().from(backlogItems).orderBy(backlogItems.priority);
    return results.map(this.toEntity);
  }

  async findById(id: string): Promise<BacklogItem | null> {
    const [result] = await db.select().from(backlogItems).where(eq(backlogItems.id, id));
  ...
  private toEntity(row: any): BacklogItem {
